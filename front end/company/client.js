...
module.exports = function(mainApp){
  mainApp.controller('UsersCtrl', [ '$scope', '$http', '$route','userService',
    function ($scope, $http, $route, userService) {
   
    $scope.currCompany = {};
    $scope.companies;	
	
    $scope.addNewChoiceBO = function() {
      var newItemNo = $scope.currCompany.beneficialOwner.length+1;
      $scope.currCompany.beneficialOwner.push({'id': newItemNo});
    };
	
	$scope.removeChoiceBO = function() {
      var lastItem = $scope.currCompany.beneficialOwner.length-1;
      $scope.currCompany.beneficialOwner.splice(lastItem);
    };
    ...
    function getAllCompany() {
      userService.getAll()
        ...
        .error(function (error) {
          console.log('Unable to load company data: ' + error.message);
        });
    }
    ...
    //Beneficial Owner - GETs selected 
    $scope.editBO = function(id) {
      $scope.editmodeBO = true;
      userService.getOneBO(id)
        .success(function(user){
          $scope.currCompany = user;
		  $scope.currCompany.beneficialOwner = [{id: '1'}];
        })
    }
	
	//GETs selected companies info, opens edit form, and populates it w/company info
    ...
    //PUT request for selected company, updates info in db and browser
    $scope.save = function(id) {
      userService.edit(id, $scope.currCompany);
      //getAllCompany();
    ...
    $scope.cancel = function() {
      $scope.editmode = false;
	  $scope.editmodeBO = false;
	  $scope.editmodeVIEW = false;
    }
  }]);
};
...
module.exports = function(mainApp){
	...
	mainApp.factory('userService', ['$http', function($http){
		return {
			//GETs all companies
			getAll: function(){
				return $http.get(hostAdress + '/Company_REST/company/all');
			},
			getOneBO: function(id){
				return $http.get(hostAdress + '/Company_REST/company/view/' + id)
			},
			getOne: function(id){
				return $http.get(hostAdress + '/Company_REST/company/view/' + id)
			},
			...
			create: function(data){
				return $http.post(hostAdress + '/Company_REST/company/add', data);
			},
			//Deletes a company
			deleteOne: function(id){
				return $http.delete(hostAdress + '/Company_REST/company/delete/' + id);
			},
			//Edits a company
			edit: function(id, data){
				return $http.put(hostAdress + '/Company_REST/company/update/' + id, data);
...
module.exports = function(mainApp){
  mainApp.controller('CreateCtrl', [ '$scope', '$http', '$location', '$route', 'userService',
                      function ($scope, $http, $location, $route, userService) {
  	$scope.user = {};
	
	$scope.user.employee = [{id: '1'}];
	
    $scope.addNewChoice = function() {
      var newItemNo = $scope.user.employee.length+1;
      $scope.user.employee.push({'id': newItemNo});
    };
	
	$scope.removeChoice = function() {
      var lastItem = $scope.user.employee.length-1;
      $scope.user.employee.splice(lastItem);
    };
   
	
    //Saves a user to the db and clears form
  	$scope.save = function(){
      /*var check = isNaN($scope.number);
      if(check){
...
function $RouteProvider(){
  ...
  this.$get = ['$rootScope',
     ...
     *
     * Note that this example is using {@link ng.directive:script inlined views}
...
function $ParseProvider() {
   ...
   * dichotomy between data access in views (accessed as raw values) and controller code
